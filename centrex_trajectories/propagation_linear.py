...
def propagate_linear(
    ...
    x = _pos(t, origin.x, velocities.vx, ax, wx)
    y = _pos(t, origin.y, velocities.vy, ay, wy)
    z = origin.z + velocities.vz * t + 0.5 * az * t**2
    vx = _vel(t, origin.x, velocities.vx, ax, wx)
    vy = _vel(t, origin.y, velocities.vy, ay, wy)
...
def propagate_linear_trajectories(
    ...
    for obj in objects:
        ...
        if not np.allclose(dz, 0):
            ...
            x, v = propagate_linear(
                t, accepted_coords, accepted_velocities, acceleration, w
            )
        else:
    ...
    if z_save is not None:
        for idz, z in enumerate(z_save):
            ...
            x, v = propagate_linear(dt, coords, vels, acceleration, w)
            if idz != 0:
